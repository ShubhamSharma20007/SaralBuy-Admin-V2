...
class Analytics {
  ...
  async getProductAnalytcs(categoryId: string) {
    return await axiosInstance
      .get('/dashboard/populate-products-by-id', {
        params: {
          categoryId,
        },
      })
  ...
  async getProductById(productId: string) {
    return await axiosInstance
      .get('/dashboard/get-product/' + productId)
      .then(res => res.data.data || res.data);
  }
  async updateProduct(formData: FormData, productId: string) {
    return await axiosInstance
      .put('/dashboard/update-product/' + productId, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      })
  ...
  async getCategories() {
    return await axiosInstance
      .get('/dashboard/get-categorie-names')
      .then(res => res.data.data || res.data);
  }
  async subCategoryProducts(categoryId: string) {
    return await axiosInstance
      .get('/dashboard/get-subcategory-count/' + categoryId)
      .then(res => res.data.data || res.data);
  }
}
