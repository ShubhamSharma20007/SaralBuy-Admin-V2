...
const BannerListing = () => {
  ...
  const {
    ...
    loading: bannerDeleteLoading,
  } = useFetch(AnalyticsInstance.deleteBanner);
  const { fn: getBannerDetsFn, data: getBannerDetsRes } = useFetch(
    AnalyticsInstance.getBanerDetailsById
  );
  const {
    fn: updateBanner,
    data: updateBannerRes,
    loading: updateBannerLoading,
  } = useFetch(AnalyticsInstance.updateBanner);
  ...
  useEffect(() => {
    if (deleteBannerData) {
      ...
      data.banners =
        data?.banners.filter((item: any) => item._id.toString() !== deleteBannerId) || [];
      setDeleteBannerId('');
      setDeleteOpen(false);
    }
  }, [deleteBannerData]);
