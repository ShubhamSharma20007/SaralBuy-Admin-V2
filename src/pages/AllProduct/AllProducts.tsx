...
const AllProducts = () => {
  const { fn: getAllProductsFn, data: getAllProductsRes } = useFetch(
    AnalyticsInstance.getAllProducts
  );
  const { fn: getProductByIdFn, data: getProductByIdRes } = useFetch(
    AnalyticsInstance.getProductById
  );
  const {
    fn: updateProductFn,
    data: updateProductRes,
    loading,
  } = useFetch(AnalyticsInstance.updateProduct);
                        ...
                        <div className="w-full">
                          ...
                          {file ? (
                            ...
                            updateForm.image && (
                              <Label className="text-sm text-gray-400 mt-2">
                                Current file:{' '}
                                {(updateForm.image as any).toString().split('/').pop()}
                              </Label>
                            )
                          )}
                        </div>
        ...
        <div className="max-w-full overflow-x-auto sm:h-[calc(100vh-250px)]">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {getAllProductsRes?.products.length !== 0 &&
                getAllProductsRes?.products.map((entry: any) => (
