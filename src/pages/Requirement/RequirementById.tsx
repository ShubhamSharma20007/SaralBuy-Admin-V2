...
const RequirementById = () => {
  ...
  const totalPages = data?.totalSellerPages || 0;
  const start = (page - 1) * limit + 1;
  const end = Math.min(page * limit, totalSellerCount);
  function handlePrev() {
    if (page > 1) {
      setPage(page - 1);
    }
            ...
            <div className="shrink-0 max-w-sm lg:max-w-sm mx-auto ">
              <div className="h-80 flex justify-center">
                <img
                  ...
                  src={productObj?.productId?.image || '/no-image.webp'}
                  alt=""
                />
                <img
          ...
          <div className="max-w-full overflow-x-auto sm:h-[calc(100vh-250px)]">
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {data?.requirement &&
                  data?.requirement?.sellers.map((entry: any) => (
                    <TableRow key={entry._id}>
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {currenySymbol(entry?.budgetAmount) || 'N/A'}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-3">
                          <div className="w-[40px] h-[40px] overflow-hidden rounded-full">
                            <img
                              className="w-full h-full object-contain"
                              src={data?.requirement?.buyerId.profileImage || '/avatar.jpg'}
                              alt={`${data?.requirement?.buyerId.profileImage} `}
                            />
                          </div>
                          <div>
                            <span
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {data?.requirement?.buyerId.phone || 'N/A'}
                      </TableCell>

                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {new Date(data?.requirement?.createdAt || '').toLocaleDateString()}
                      </TableCell>
                    </TableRow>
                  ))}
              </TableBody>
          ...
          <div className="flex items-center justify-between border-t border-white/10 px-4 py-3 sm:px-6">
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <p className="text-sm text-gray-800 dark:text-white">
                ...
                of
                <span className="font-medium px-1">{totalSellerCount}</span>
                results
              </p>
                    ...
                    <button
                      ...
                      className={`relative inline-flex items-center dark:text-white px-4 py-2 text-sm font-semibold ${
                        page === num
                          ...
                          : ' text-gray-700 dark:hover:bg-gray-500'
                      }`}
                    >
                      {num}
                    </button>
...
export default RequirementById;
