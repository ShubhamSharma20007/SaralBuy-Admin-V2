...
const Requirement = () => {
  ...
  const totalRequirements = data?.totalRequirements || 0;
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {data &&
                ...
                data?.requirements.map((entry: any) => {
                  return (
                    <TableRow key={`${entry?._id}-${entry?.productId?._id}`}>
                      <TableCell className="px-5 py-4 sm:px-6 text-start">
                        <div className="flex items-center gap-3">
                          <div className="w-[40px] h-[40px] overflow-hidden rounded-full">
                            <img
                              className="w-full h-full object-contain"
                              src={entry?.productId?.image || '/no-image.webp'}
                              alt={`${entry?.productId?.title} `}
                            />
                          </div>
                          <div>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {entry?.productId?.brand || 'N/A'}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {currenySymbol(entry?.productId?.minimumBudget)}
                      </TableCell>

                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        {entry?.sellers?.length || 0}
                      </TableCell>
                      {/* <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    ...
                    >
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 flex items-center gap-2">
                        <div
                          onClick={() => {
                            navigate('/product-requirement/' + entry._id);
                          }}
        ...
        <div className="flex items-center justify-between border-t border-white/10 px-4 py-3 sm:px-6">
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <p className="text-sm text-gray-800 dark:text-white">
              ...
              of
              <span className="font-medium px-1">{totalRequirements}</span>
              results
            </p>
